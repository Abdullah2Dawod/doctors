...
<div class="container-fluid">
    ...
    <table class="table table-striped">
        ...
        <tbody>
            @foreach ($Surgery as $index=> $Surgerys)
          <tr>

            <th scope="row">{{++$index}}</th>
            <td>{{$Surgerys->name}}</td>
            <td>{{$Surgerys->university}}</td>
            <td>{{$Surgerys->addres}}</td>
            <td>{{$Surgerys->price}}</td>
            <td>{{$Surgerys->mobaile}}</td>
            <td>
                <div style="display: flex;">
                    <div>
                        <a href="{{route('edit.doctor' , $Surgerys->id )}}">
                            <i class="fa fa-pencil-square" aria-hidden="true"></i>
                    </a>
                </div>
                <div>
                    <a href="{{route('destroy.doctor' , $Surgerys->id)}}" style="margin-right: 30px !important">
