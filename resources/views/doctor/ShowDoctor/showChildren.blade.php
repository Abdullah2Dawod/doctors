...
<div class="container-fluid">
    ...
    <table class="table table-striped">
        ...
        <tbody>
            @foreach ($Children as $index=> $Childrens)
          <tr>

            <th scope="row">{{++$index}}</th>
            <td>{{$Childrens->name}}</td>
            <td>{{$Childrens->university}}</td>
            <td>{{$Childrens->addres}}</td>
            <td>{{$Childrens->price}}</td>
            <td>{{$Childrens->mobaile}}</td>
            <td>
                <div style="display: flex;">
                    <div>
                        <a href="{{route('edit.doctor' , $Childrens->id )}}">
                            <i class="fa fa-pencil-square" aria-hidden="true"></i>
                    </a>
                </div>
                <div>
                    <a href="{{route('destroy.doctor' , $Childrens->id)}}" style="margin-right: 30px !important">
