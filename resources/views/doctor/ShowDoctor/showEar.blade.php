...
@section('title2')
مدير النظام - أطباء أذن-أنف-حنجرة
@endsection
...
<div class="container-fluid">
    ...
    <table class="table table-striped">
        ...
        <tbody>
            @foreach ($Ear as $index=> $Ears)
          <tr>

            <th scope="row">{{++$index}}</th>
            <td>{{$Ears->name}}</td>
            <td>{{$Ears->university}}</td>
            <td>{{$Ears->addres}}</td>
            <td>{{$Ears->price}}</td>
            <td>{{$Ears->mobaile}}</td>
            <td>
                <div style="display: flex;">
                    <div>
                        <a href="{{route('edit.doctor' , $Ears->id )}}">
                            <i class="fa fa-pencil-square" aria-hidden="true"></i>
                    </a>
                </div>
                <div>
                    <a href="{{route('destroy.doctor' , $Ears->id)}}" style="margin-right: 30px !important">
