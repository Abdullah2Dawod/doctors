...
@section('title2')
مدير النظام - أطباء العلاج الطبيعي
@endsection
...
<div class="container-fluid">
    ...
    <table class="table table-striped">
        ...
        <tbody>
            @foreach ($Physical as $index=> $Physicals)
          <tr>

            <th scope="row">{{++$index}}</th>
            <td>{{$Physicals->name}}</td>
            <td>{{$Physicals->university}}</td>
            <td>{{$Physicals->addres}}</td>
            <td>{{$Physicals->price}}</td>
            <td>{{$Physicals->mobaile}}</td>
            <td>
                <div style="display: flex;">
                    <div>
                        <a href="{{route('edit.doctor' , $Physicals->id )}}">
                            <i class="fa fa-pencil-square" aria-hidden="true"></i>
                    </a>
                </div>
                <div>
                    <a href="{{route('destroy.doctor' , $Physicals->id)}}" style="margin-right: 30px !important">
