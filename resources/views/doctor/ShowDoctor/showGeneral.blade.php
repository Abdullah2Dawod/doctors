...
<div class="container-fluid">
    ...
    <table class="table table-striped">
        ...
        <tbody>
            @foreach ($General as $index=> $Generals)
          <tr>

            <th scope="row">{{++$index}}</th>
            <td>{{$Generals->name}}</td>
            <td>{{$Generals->university}}</td>
            <td>{{$Generals->addres}}</td>
            <td>{{$Generals->price}}</td>
            <td>{{$Generals->mobaile}}</td>
            <td>
                <div style="display: flex;">
                    <div>
                        <a href="{{route('edit.doctor' , $Generals->id )}}">
                            <i class="fa fa-pencil-square" aria-hidden="true"></i>
                    </a>
                </div>
                <div>
                    <a href="{{route('destroy.doctor' , $Generals->id)}}" style="margin-right: 30px !important">
